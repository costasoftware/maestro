import type { AgentToolDefinition, BaseToolContext } from '@costasoftware/maestro-core'
 ...
 * `@costasoftware/maestro-core` tool, just with a static `mockOutput` for offline
 * runs.
 */
export interface EvalFixture<TCtx extends BaseToolContext<string> = BaseToolContext> {
     ...
     * @costasoftware/maestro-core's `BuildAiSdkToolsArgs.registry`) so heterogeneous
