...
- `@costasoftware/eval` — promptfoo harness for golden paths + injection (now shipped as the live `@costasoftware/maestro-evals`)
- `@costasoftware/maestro-telemetry` — typed plane client (v1 ships as `@costasoftware/maestro-core` subpath)
...
| --- | --- | --- | --- | --- |
| **P1** | `maestro/` (this repo) | none | Bootstrap: pnpm + turbo + tsconfig + CI. Empty `@costasoftware/maestro-core` skeleton with envelope only. `0.0.1`. | — |
| **P2** | `maestro/` | P1 | Lift `defineAgentTool` factory, `BaseToolContext`, safe-tool, cache-control. Adapter subpath: `@costasoftware/maestro-core/adapters/ai-sdk` + `.../adapters/mcp-server`. Port INTERFACES (no impls). `0.1.0`. | — |
